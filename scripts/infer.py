...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--save-probs", default=None, help="Optional path to save per-input scores")
...
def main() -> None:
    ...
    effective_cfg = {
        "output": {
            ...
            "probs_path": str(out_probs) if out_probs else None,
        },
        "input": {
    ...
    for fpath in tqdm(files):
        ...
        if ext in SUPPORTED_IMG:
            ...
            if args.save_probs or out_probs:
                score_rows.append({"video_id": fname, "score": float(score)})
            continue
    ...
    target: Path | None
    if args.save_probs:
        target = Path(args.save_probs)
    else:
        target = out_probs

    if target is not None:
        target.parent.mkdir(parents=True, exist_ok=True)
        pd.DataFrame(score_rows, columns=["video_id", "score"]).to_csv(target, index=False)
        logger.info("Saved scores => %s", target)
